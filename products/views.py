...
def all_products(request):
    ...
    if request.GET:
        ...
        # show specific categories of products
        if 'category' in request.GET:
            categories = request.GET['category'].split(',')
            products = products.filter(category__name__in=categories)
            categories = Category.objects.filter(name__in=categories)

        # search query functionality using product name or product descriptions
        if 'q' in request.GET:
            query = request.GET['q']
            if not query:
                messages.error(
...
def add_product(request):
    ...
    if not request.user.is_superuser: # this can be done with another decorator like @login_required or a custom one can be made
